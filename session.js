...
passport.use(new LocalStrategy(
 async function(username, password, done) {
  if (username.length <= 13 && password.length <=13){
    const users = await sequelize.query(`select id,nombre,usuario,telefono,nivel,puesto,departamento,__acceso_portal_web from mega_shoes_data.dbo.modusers m where usuario='${username}' 
